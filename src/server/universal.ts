...
import { SERVER_GRAPHQL_CONFIG } from '@lib/graphql';
...
// Leave this as require() since this file is built Dynamically from webpack, path at build time
...
export const htmlEngine: (port: number) => Function = (port) => (_, options, callback) => {
   renderModuleFactory(AppServerModuleNgFactory, {
      document: template,
      url: options.req.url,
      extraProviders: [
         provideModuleMap(LAZY_MODULE_MAP),
         { provide: SERVER_GRAPHQL_CONFIG, useValue: { origin: `http://localhost:${port}` } },
      ],
   })
